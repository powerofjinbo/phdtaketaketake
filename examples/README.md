...
|---|---|---|
| [`physics_hep_strict/`](physics_hep_strict/) | physics / HEP | Full 4-stage pipeline (discovery_plan → collect_evidence → audit → match) on a Tsinghua undergrad applying to top-10 US physics PhDs with ATLAS / Higgs background. 3 candidates differentiated to land in `target` / `reach` / `only_if_space` strategy buckets. |
...
3. Add `fixtures/<adapter>/{find_author,recent_works,coauthored}/*.json`
   ...
   for the layout
4. Write `run_example.sh` (copy from physics_hep_strict, adjust args)
