...
async function installCmd(args, { packageRoot, env, console: out }) {
  ...
  try {
    ...
    // Unknown / malformed flag → friendly error, not stack trace.
    out.error(e.message);
    out.error('');
    out.error(HELP_TEXT);
    return 2;
  ...
  // Cursor requires --project (Project Rules are project-scoped). When
  // --all is used without --project, skip Cursor with a friendly note
  // rather than letting it fail and pollute the overall exit code. The
  // user can re-run `--cursor --project .` separately when they want it.
  const targets = [];
  for (const t of wantedTargets) {
    if (t === 'cursor' && !flags.projectDir) {
      if (flags.all) {
        out.log('\n--- Skipping cursor (requires --project) ---');
        out.log('  Cursor Project Rules are scoped to a project tree. To install:');
        out.log('  npx @powerofjinbo/phdtaketaketake install --cursor --project .');
        continue;
      }
      // Explicit --cursor without --project: let installCursor() emit
      // its own exit-2 error message (preserves backwards-compat with
      // the c5 test suite).
    }
    targets.push(t);
  }
...
export function detectClaudeCli({ env, runner, platform } = {}) {
  const run = runner || ((cmd, args) => spawnSync(cmd, args, { env }));
  // `which` on POSIX, `where` on Windows. Both write the resolved path
  // to stdout and exit 0 when the binary is found.
  const detectorCmd = (platform || process.platform) === 'win32' ? 'where' : 'which';
  const result = run(detectorCmd, ['claude']);
  if (result.status === 0 && result.stdout) {
    // `where` on Windows can return multiple lines (once per PATH match);
    // take the first one.
 ...
 *
 * Codex discovers skills under `.agents/skills/<skill-name>/` (project-
 * local) and `$HOME/.agents/skills/<skill-name>/` (user-level). The
 * earlier draft of this installer wrote to `~/.codex/skills/`, which is
 * the wrong path — `~/.codex/AGENTS.md` is global Codex *guidance*, not
 * a skill discovery location. See review feedback that prompted c4.
 *
 * Default: install to `$HOME/.agents/skills/phdtaketaketake/`.
 * With `--project <dir>`: install to `<dir>/.agents/skills/phdtaketaketake/`
 * for repo-local skill availability inside a specific Codex workspace.
 *
 * AGENTS.md at our package root is a short pointer to SKILL.md (not a
 * copy); it ships into the skill directory alongside SKILL.md so
 * Codex's AGENTS.md discovery picks up either entry-point file.
 */
export async function installCodex({
  ...
  const baseDir = projectDir
    ? resolve(projectDir)
    : env.AGENTS_HOME || homedir();
  const skillsRoot = projectDir
    ? join(baseDir, '.agents', 'skills')
    : join(baseDir, '.agents', 'skills');
  const dest = join(skillsRoot, 'phdtaketaketake');

  out.log(`Codex skill destination: ${dest}`);
  if (projectDir) {
    out.log('  (project-local install — only this Codex workspace will see this skill)');
  } else {
    out.log('  (user-level install — every Codex workspace on this machine will see this skill)');
  }
  ...
  copyPackage(packageRoot, dest);
  out.log(`✓ Copied package → ${dest}`);
  out.log('  (AGENTS.md ships in the package root and points at SKILL.md;');
  out.log('   Codex picks up either as the skill entry point.)');
...
/**
 * Cursor Project Rule generator.
 *
 * Cursor's modern persistent-instructions surface is `.cursor/rules/*.mdc`
 * (Project Rules), with frontmatter declaring `description` and
 * `alwaysApply` (boolean). Modern Cursor also reads `AGENTS.md`, so a
 * project that ran `phdtake install --cursor --project .` AND
 * `phdtake install --codex --project .` would have skill discovery
 * via both paths.
 *
 * The legacy `.cursorrules` file is still respected by older Cursor
 * versions; if a project wants that fallback they can copy the
 * generated .mdc body. We don't write `.cursorrules` automatically
 * because it conflicts with the Project Rules system in newer Cursor.
 *
 * The .mdc we generate is a SHORT POINTER + invariants summary, not a
 * copy of SKILL.md — same drift-avoidance rationale as AGENTS.md.
 */
export async function installCursor({
  ...
  if (!packageRoot) packageRoot = PACKAGE_ROOT;
  if (!projectDir) {
    out.error('--cursor requires --project <path>. Cursor Project Rules are scoped to a project tree.');
    return 2;
  }
  const project = resolve(projectDir);
  const rulesDir = join(project, '.cursor', 'rules');
  const dest = join(rulesDir, 'phdtaketaketake.mdc');

  mkdirSync(rulesDir, { recursive: true });

  const skillReadable = existsSync(join(project, 'SKILL.md'))
    ? './SKILL.md'
    : '<path-to-installed-phdtaketaketake>/SKILL.md';

  const mdc = renderCursorMdc({ skillPath: skillReadable });
  writeFileSync(dest, mdc);
  out.log(`✓ Wrote Cursor project rule → ${dest}`);
  out.log('  (Cursor reads .cursor/rules/*.mdc as Project Rules; alwaysApply=false');
  out.log('  means it activates when the agent recognizes a relevant request.)');

  if (skillReadable !== './SKILL.md') {
    out.log('');
    out.log('  Note: SKILL.md was not found at the project root. Either:');
    out.log('    - install phdtaketaketake into the project (npm install --save-dev or copy)');
    out.log('    - or update the SKILL.md path inside the .mdc by hand');
  }

  return 0;
}

/** Render the Cursor Project Rule body. Pure function for testability. */
export function renderCursorMdc({ skillPath = './SKILL.md' } = {}) {
  return `---
description: Use phdtaketaketake for evidence-first PhD advisor matching, application triage, and CV optimization.
alwaysApply: false
---

# phdtaketaketake — Cursor Project Rule

Trigger this skill when the user asks for any of:

- PhD advisor matching / PI ranking / application triage
- connection-first evaluation between the user's current advisor and target PIs
- evidence-backed application strategy
- PhD-application CV editing or LaTeX CV compilation

Canonical instructions live in [\`SKILL.md\`](${skillPath}). Read that file
before acting; this rule is a short pointer, not a substitute.
...
per-section CV editing conventions are in \`SKILL.md\` and
\`references/\`. This rule does not duplicate them.
`;
}
