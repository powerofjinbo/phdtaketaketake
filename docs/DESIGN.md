...
| 8 | CI / packaging / distribution | ✅ done (Sprint-3-c5) — CI workflow lives at `.github/workflows/ci.yml` (lint + mypy + tests + console-script smoke across py3.11/3.12); `pyproject.toml` exposes 4 console scripts (`phdtaketaketake-match` / `-audit` / `-collect-evidence` / `-discovery-plan`); script bodies moved to `phd_matcher/cli/` package, `scripts/<name>.py` thin shims for checkout-only invocation. PyPI publishing deferred (`<COMMIT-PENDING>`) |
