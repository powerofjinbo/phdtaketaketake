...
| Source cache + rate limit (Sprint-3-c4) | `CachedAdapter` + `RateLimitedAdapter` decorators | ✅ done — disk-cached JSON via MD5 keys per (op, args); optional `ttl_seconds`; preserves inner adapter's `name` and forwards `errors`. Rate-limited adapter throttles live calls (default 0.1s polite-pool friendly). Composable: cache wrapping rate-limit means cache hits skip the wait entirely. CLI gains `--cache-dir`, `--cache-ttl-days`, `--rate-limit-seconds`; output `mode` reports `live+cache` etc. (`<COMMIT-PENDING>`) |
