...
| Research Fit v2 (Sprint-2-c3) | Structured 6-axis weighted score | ✅ done — `ResearchFit` submodel with 6 weighted axes (topic 0.30 / method 0.20 / system 0.15 / temporal 0.15 / grant 0.10 / background 0.10) replaces the free-form `research_fit_axes` dict; `theory_experiment_fit` stored for display, NOT in formula; resolution priority: v2 ResearchFit → legacy `research_fit_score` → None; tie-breaker role unchanged (`<COMMIT-PENDING>`) |
